...
async function testQuery(conn, sql, params = [], opacity = { outFormat: oracledb.OUT_FORMAT_OBJECT }) {
    try {
        const result = await conn.execute(sql, params, opacity);
        return result.rows;
    } catch (err) {
...
async function main() {
    ...
    // 3. 查询 pb_dept 表结构
    console.log('\n>>> 查询 pb_dept 表结构...');
    const deptCols = await describeTable(conn, 'PB_DEPT');
    console.log('字段列表:');
    ...
    const testSql1 = `SELECT * FROM pb_dept_member WHERE ROWNUM <= 3`;
    constrows1 = await testQuery(conn, testSql1);
    if (rows1) {
        console.log('数据示例:');
    ...
    const sqlRate = `SELECT COUNT(*) as cnt FROM ord_bas 
                     WHERE created >= TO_DATE(:dateFrom, 'YYYY-MM-DD') 
                       AND created < TO_DATE(:dateTo, 'YYYY-MM-DD') 
                       AND isactive = 'Y'`;
    const rows5 = await testQuery(conn, sqlRate, [
