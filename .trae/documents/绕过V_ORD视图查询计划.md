...
根据 MCP 查询结果，新厂数据库包含以下关键表：

### 1. 订单主表：ORD_CT
- 字段：ID, SERIAL, CLIENTID, ORGCDE, CREATED, ISACTIVE, AGNTCDE（可能为空）, ...

### 2. 订单基础表：ORD_BAS
- 字段：ID, SERIAL, CLIENTID, ORGCDE, CLNTCDE, PRDCDE, PRDNME, ACCNUM, PRICES, PTDATE, QUOPRC, ...

### 3. 客户表：PB_CLNT
- 字段：CLIENTID, ORGCDE, CLNTCDE, CLNTNME, AGNTCDE（业务员编码）, SMNME, SMPING, ...

### 4. 产品表：需要确认表名
- 可能为：PB_PRD 或 PB_PRD_CT

### 5. 员工表：HR_BASE
- 字段：EMPCDE, EMPNME, ...

### 6. 部门/团队表：PB_TEAM, PB_TEAM_EMP
- 用于筛选销售1部的业务员
...
SELECT 
    ...
    c.CLNTNME as 客户,
    b.PRDNME as 产品,
    c.AGNTCDE as 业务员编码,  -- 从 pb_clnt 表获取
...
LEFT JOIN PB_CLNT c ON b.CLIENTID = c.CLIENTID AND b.ORGCDE = c.ORGCDE AND b.CLNTCDE = c.CLNTCDE AND c.OBJTYP = 'CT'
WHERE t.ISACTIVE = 'Y'
  AND b.PTDATE >= TO_DATE('2026-03-07', 'yyyy-MM-dd')
  AND b.PTDATE < TO_DATE('2026-03-08', 'yyyy-MM-dd')
  -- 业务员筛选：从 pb_clnt.agntcde 获取
...
1. **验证表结构**：确认 ORD_CT、ORD_BAS、PB_CLNT 表结构
2. **验证字段映射**：确认 pb_clnt.agntcde 存储业务员编码
3. **测试查询**：使用 MCP 测试新 SQL 是否能正常执行
4. **对比结果**：与 V_ORD 视图查询结果对比
